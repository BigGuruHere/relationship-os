...
| `userId` as the sole custody predicate for Core knowledge | explicit ownership + `ContextSpace` custody, scoped by `(userId, contextSpaceId)` | 8.6 | **Completed for current Core/Workspace custody paths in 8.6** | **Default-space foundation completed 8.6; future context-switch UI must use the same scope primitive** | Every existing User has one default ContextSpace; Core reads require both ids; Workspace/request reads are narrowed to active context; raw SQL and inherited agent/tag paths are context-scoped; same-owner/different-context behavioural checks pass |
