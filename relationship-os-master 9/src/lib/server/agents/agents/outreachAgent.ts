...
function normaliseCandidate(raw: OutreachCandidateOutput): OutreachCandidateOutput | null {
  ...
  return {
    ...
    strategicFitScore: clampScore(raw.strategicFitScore, 50),
    valuePotentialScore: clampScore(raw.valuePotentialScore, 50),
    relationshipPathScore: clampScore(raw.relationshipPathScore, 50),
    evidenceQualityScore: clampScore(raw.evidenceQualityScore, 50),
    riskScore: clampScore(raw.riskScore, 50),
    scoreLabel: cleanText(raw.scoreLabel),
    priority: cleanText(raw.priority),
    recommendedAction: cleanMultiline(raw.recommendedAction),
    scoreFactors: Array.isArray(raw.scoreFactors) ? raw.scoreFactors.slice(0, 12) : [],
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      const score = await executeAgentTool<any, any>('create_opportunity_score', {
        ...
        strategicFitScore: candidate.strategicFitScore,
        valuePotentialScore: candidate.valuePotentialScore,
        relationshipPathScore: candidate.relationshipPathScore,
