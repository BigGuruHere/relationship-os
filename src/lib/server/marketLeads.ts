...
export async function convertLeadToDeal(userId: string, leadId: string) {
  ...
  const deal = await prisma.deal.create({
    data: {
      ...
      valueCents: lead.valueMaxCents || lead.valueMinCents || null,
...
export async function convertLeadToExchangeItem(userId: string, leadId: string, type: 'WANT' | 'OFFER') {
  ...
  if (type === 'WANT') {
    const want = await prisma.want.create({
      data: {
        ...
        authority: 'THIRD_PARTY_REPORTED' as any,
        sourceType: 'MANUAL' as any,
        sourceNoteEnc: encrypt('Created from Market Lead', 'want.source_note'),
        geographyEnc: display.geography ? encrypt(display.geography, 'want.geography') : null,
        valueMinCents: lead.valueMinCents || null,
        valueMaxCents: lead.valueMaxCents || null,
        currency: display.currency || 'AUD',
  ...
  const offer = await prisma.offer.create({
    data: {
      ...
      authority: 'THIRD_PARTY_REPORTED' as any,
      sourceType: 'MANUAL' as any,
      sourceNoteEnc: encrypt('Created from Market Lead', 'offer.source_note'),
