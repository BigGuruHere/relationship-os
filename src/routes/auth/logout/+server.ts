...
export const POST: RequestHandler = async ({ locals, cookies, url, request }) => {
  // IT: destroy server-side session if present - errors are non-fatal
  if (locals.sessionId) {
    try {
      await destroySession(locals.sessionId);
    } catch (e) {
      // IT: do not block logout if the session was already gone
      console.warn('destroySession failed during logout:', e);
    }
  }

  // IT: clear the browser cookie - env-aware name and flags from locals
  clearSessionCookie(cookies, locals);

  // IT: allow caller to specify where to go next, else default to login
  ...
  // IT: 303 is the correct redirect after POST
  throw redirect(303, next);
};

// IT: optional - if someone visits /auth/logout with GET, just bounce them away without side effects
