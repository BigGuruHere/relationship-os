<!-- src/routes/deals/[id]/relationships/[linkId]/notes/new/+page.svelte -->
<script lang="ts">
  // PURPOSE: Add a note to one deal-person commercial thread with optional voice recording, transcription, and AI summary.
  ...
  export let form: any;
  export let data: {
    thread: { id: string; dealId: string; dealTitle: string; contactId: string; contactName: string; company: string };
  };
  ...
  async function summarize() {
    try {
      ...
      console.error('[deal-thread-note] summarize failed', err);
    } finally {
      summarizing = false;
    }
  }
    ...
    try {
      ...
      mediaRecorder.ondataavailable = async (e) => {
        ...
        for (let o = 0; o < full.length; o += CHUNK) {
          ...
          if (!(status >= 200 && status < 300) || data?.ok !== true) {
            console.error('[deal-thread-note] stream chunk failed', { status, data });
          }
        }
      };
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          console.error('[deal-thread-note] transcribe failed', err);
        } finally {
          transcribing = false;
          recording = false;
      ...
      mediaRecorder = null;
      recording = false;
      console.error('[deal-thread-note] start recording failed', err);
    }
  }
  ...
  function stopRecording() {
    ...
    try {
      ...
      console.error('[deal-thread-note] stop recording failed', err);
    }
  }
  ...
  async function handleCancel() {
    hardStopRecording();
    await goto(`/deals/${data.thread.dealId}/relationships/${data.thread.id}`);
  }
</script>

<div class="container">
  <div class="card note-card">
    <h1>Add thread note</h1>
    <div class="muted top-meta">for <a href={`/deals/${data.thread.dealId}/relationships/${data.thread.id}`}>{data.thread.contactName}</a> on <a href={`/deals/${data.thread.dealId}`}>{data.thread.dealTitle}</a></div>
    ...
    <form method="post" on:submit={handleSubmit}>
      ...
      <div class="field">
        <label for="text">Thread note</label>
      ...
      <div class="button-row">
        ...
        <button class="btn primary" formaction="?/save" formmethod="post" disabled={saving || transcribing || recording}>
          {#if saving}<span class="spinner" aria-hidden="true"></span><span>Saving...</span>{:else}<span>Save thread note</span>{/if}
        </button>
