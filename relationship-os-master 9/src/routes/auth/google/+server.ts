...
// IT: build a code challenge from a verifier
...
export const GET: RequestHandler = async ({ locals, cookies, url }) => {
  ...
  // IT: generate CSRF and replay protection values
  const state = b64url(randomBytes(16));
  const nonce = b64url(randomBytes(16));
  const codeVerifier = b64url(randomBytes(32));
  const codeChallenge = toCodeChallenge(codeVerifier);

  // IT: short lived cookie attributes - secure flag comes from env-aware session cookie config
  const baseCookie = {
    path: '/',
    httpOnly: true,
  ...
  // IT: store temporary OAuth cookies
  ...
  // IT: optional prompt and access settings
  const prompt = url.searchParams.get('prompt') || 'consent'; // allow override if needed

  // IT: build the Google authorization URL
