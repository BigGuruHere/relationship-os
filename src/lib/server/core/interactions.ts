...
import {
	findAccessibleCorePerson,
	findCoreCompany,
	findCoreContact,
	findCoreInteraction
} from '$lib/server/core/relationshipRepository';
import {
	decryptInteractionSummary,
	encryptInteractionSummary
} from '$lib/server/core/interactionEncryption';
...
export async function createCoreInteraction(
	context: CoreAccessContext,
	input: CreateCoreInteractionInput
) {
	...
	// IT: Dating pilot transcripts opt out because an embedding is not required to review or approve an Outcome.
	if (input.createEmbedding !== false) {
		...
		try {
			...
			console.warn('[core:interaction] failed to store private interaction embedding', {
