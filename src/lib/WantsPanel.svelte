...
<section class="card wants-panel">
  ...
  {#if showForm}
    <form method="post" action="?/createWant" class="nested-form want-form">
      ...
      <div class="grid four">
        <div class="field"><label for="valueMin">Value min</label><input id="valueMin" name="valueMin" inputmode="decimal" placeholder="e.g. 1000000" /></div>
        <div class="field"><label for="valueMax">Value max</label><input id="valueMax" name="valueMax" inputmode="decimal" placeholder="e.g. 5000000" /></div>
    ...
    <div class="want-list">
      {#each items as item}
        <div class="want-item">
          ...
          <form method="post" action="?/deleteWant" on:submit={(event) => { if (!confirm('Delete this want?')) event.preventDefault(); }}>
