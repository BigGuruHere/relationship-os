...
<script lang="ts">
	...
	export let uploadEndpoint = '/api/upload-chunk';
	export let resultEndpoint = '/api/transcribe-result';

	// IT: Preserve the older `name` prop while preferring the explicit textName prop for new callers.
	$: resolvedTextName = name || textName;
	...
	async function pollResult(jobId: string): Promise<string> {
		for (let i = 0; i < 60; i++) {
			const res = await fetch(`${resultEndpoint}?jobId=${encodeURIComponent(jobId)}`);
			const json = await res.json().catch(() => ({}));
	...
	async function startRecording() {
		...
		try {
			...
			mediaRecorder.ondataavailable = (event) => {
				...
				// IT: MediaRecorder events can overlap. Serialise uploads so the final marker cannot overtake audio chunks.
				uploadQueue = uploadQueue.then(async () => {
					...
					for (let offset = 0; offset < full.length; offset += chunkSize) {
						...
						const { status, data } = await uploadChunk(uploadKey, index, false, part);
						if (!(status >= 200 && status < 300) || data?.ok !== true) {
							throw new Error(`Audio chunk ${index} failed to upload.`);
						}
					}
				});
			};
			...
			mediaRecorder.onstop = async () => {
				transcribing = true;
				try {
					// IT: Wait for every queued audio chunk before marking the recording complete.
					await uploadQueue;
...
<div class="voice-field">
		...
		<div class="voice-actions">
			...
			{:else if isMobile}
				<span class="inline-wait"
					><span class="spinner" aria-hidden="true"></span><span>Recording...</span></span
				>
			{:else}
				<button type="button" class="btn" on:click={stopRecording} {disabled}>⏹ Stop</button>
			{/if}
	...
	{#if transcribing}
		<div class="inline-wait">
			<span class="spinner" aria-hidden="true"></span><span>Transcribing {contextLabel}...</span>
		</div>
	{/if}
...
{#if recording && isMobile}
	<RecordingGuard visible {transcribing} on:stop={stopRecording} />
{/if}
