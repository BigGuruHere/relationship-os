...
export const load: PageServerLoad = async ({ params, locals }) => {
  ...
  const offers = await loadOffers({ userId: locals.user.id, links: { dealId: row.id } });
...
export const actions: Actions = {
  ...
  createOffer: async ({ request, params, locals }) => {
    ...
    try {
      await createOfferFromForm({ userId, form: await request.formData(), links: { dealId: params.id } });
    } catch (err: any) {
      console.error('[deals:createOffer] failed', err);
  ...
  deleteOffer: async ({ request, params, locals }) => {
    ...
    await deleteOffer({ userId: locals.user.id, id: offerId, links: { dealId: params.id } });
    throw redirect(303, `/deals/${params.id}`);
  },
